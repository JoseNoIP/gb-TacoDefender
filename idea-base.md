...
- **UI in-game (HUD/paneles):** el proyecto no tiene ningún `Theme` propio — TODO Button/Label/PanelContainer usa el tema plano default de Godot. Los íconos de torre en la barra inferior ya usan el arte nuevo (`HUD._build_bottom_bar()`); el resto (barra superior de oro/vida/oleada, panel de selección de torre, Pause/GameOver/Victory, toast, FTUE) sigue con controles default sin estilizar. Ver conversación de sesión para el detalle priorizado.
