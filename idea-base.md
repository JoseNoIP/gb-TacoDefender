...
- **UI in-game (HUD/paneles):** RESUELTO en gran parte. `assets/theme/game_theme.tres` (generado por `tools/build_game_theme.gd`, referenciado en `project.godot` `[gui] theme/custom`) reestiliza TODOS los Button/Label/PanelContainer del juego (antes: tema plano default de Godot). Íconos agregados: moneda de oro + corazón de vida en la barra superior (`tools/gen_ui_icons.py`, procedural), ícono de torre en el panel de selección y en los botones de compra (arte de `fetch_taco_object_sprites.py`). Pendiente: imagen/celebración en Pause/GameOver/Victory (hoy solo texto con el theme nuevo), animación de entrada/salida del toast.
