...
- **Arte real:** RESUELTO. Fondos de pantalla completa (menú, detrás del tablero, tienda de mejoras) generados con Pollinations.ai vía `tools/fetch_taco_backgrounds.py`, integrados con `src/shared/background_style.gd` (velo oscuro para legibilidad de texto — ver `/gen-ai-art` Paso 7). Torres/enemigos/proyectiles reemplazados por sprites procedurales (`tools/gen_taco_sprites.py`, mismo generador pixel-art de `gen_assets.py`, sin IA — los tamaños en juego, 16-40px, caen en la zona "procedural mejor que IA" del skill). Icono y splash ya estaban resueltos proceduralmente desde el build inicial.
