...
65. **Un mapa/tablero/nivel más alto que el hueco visible entre HUD superior y barra inferior (cualquier juego con scroll/pan de cámara — no solo tower defense) necesita que ALGO mueva la cámara automáticamente en el momento en que importa (spawn, un enemigo/objetivo acercándose al final), o de lo contrario esa zona queda invisible detrás de una barra opaca en la posición de cámara POR DEFECTO (con la que arranca cada partida) hasta que el jugador arrastre manualmente — y un jugador que nunca arrastra nunca la ve.** Bug real: un tablero de 14 filas (840px) era ~236px más alto que el hueco real entre HUD y barra inferior (604px); la cámara solo se movía por drag manual (`_on_drag()`), nunca auto-scroll — las últimas 2 vueltas del camino Y el punto de llegada quedaban literalmente dibujados detrás de la barra inferior en la posición inicial. Síntoma reportado por el jugador: "la barra tapa el camino" + "el Game Over tarda en aparecer" — ambos eran el mismo bug (el enemigo seguía caminando de verdad ahí atrás, invisible), no dos problemas distintos. Confirmado con matemática (altura tablero vs hueco real) Y con un probe real instrumentado temporalmente que logueó la posición Y de cámara antes/después de varios segundos de movimiento sin input de drag (quedó exactamente igual). Dos fixes válidos según el caso: (a) agregar auto-scroll de cámara hacia el objetivo relevante, o (b) si el contenido lo permite, reducir el tablero/nivel para que quepa completo en el hueco visible — con esto último, `compute_camera_bounds()` (o el cálculo equivalente de límites de cámara) debe colapsar min/max al mismo punto (nunca asumir un rango de pan que ya no aplica; un test que afirmaba "el tablero real necesita pan" se volvió falso y tuvo que reescribirse). Regla general: cualquier feature con drag-to-pan sobre un área de juego debe verificarse explícitamente contra la altura REAL del hueco visible (HUD_TOP + BOTTOM_BAR restados del viewport, mismo cálculo que la regla #61) — no asumir que "cabe" solo porque el editor lo muestra sin recortar.
66. **Cuando un nodo (Board, un nivel, cualquier escena) elige su propia configuración interna (qué mapa/template/variante usar) en `_ready()` leyendo un autoload persistente real (`MetaManager.get_victories()`, o cualquier progreso guardado), los tests de ESE nodo NUNCA deben hardcodear coordenadas/valores que solo son válidos para UNA configuración específica — el autoload en GUT es el MISMO real usado por partidas jugadas a mano (mismo espíritu que la regla #57, pero acá el problema no es que el test corrompa el save, sino que el test da FALSOS POSITIVOS/NEGATIVOS dependiendo de cuánto haya jugado la persona que corre la suite).** Bug evitado en este proyecto: al agregar 10 templates de camino distintos elegidos según `victories % 10`, los tests de `Board` que asumían `(1,1)` como "la celda construible de siempre" hubieran pasado o fallado según las victorias REALES guardadas en `meta.json` en la máquina de quien corriera `run_tests.sh` — determinístico en apariencia, pero en realidad dependiente de progreso de juego incidental, no de la lógica que se quiere probar. Fix: el nodo expone un getter de su estado real ya resuelto (`Board.get_path_cells() -> Dictionary`) y los tests derivan la celda de interés en tiempo de ejecución a partir de ESE getter (ej. "la primera celda construible que encuentre recorriendo la grilla"), nunca de una coordenada fija pensada para una sola configuración. Regla general: si el comportamiento de un nodo bajo test depende de un autoload con estado persistente real, los tests deben ser correctos para CUALQUIER valor de ese estado, no solo para el que tenía la máquina de quien los escribió.
67. **El "safe zone" de un ícono adaptativo de Android (`launcher_icons/adaptive_foreground_432x432` en `export_presets.cfg`) es un CÍRCULO centrado de 66% de diámetro, no un cuadrado — escalar el sujeto para que su bounding box cuadrado ocupe ese 66% del canvas deja sus ESQUINAS fuera del círculo (un cuadrado de lado 66% tiene semi-diagonal ≈0.47 del canvas, mientras que el radio del círculo seguro es 0.33), así que cualquier parte del sujeto cerca de las esquinas de su propio bounding box (brazos extendidos, apéndices diagonales) puede recortarse en launchers con máscara circular aunque "se vea bien" al mirar el PNG plano sin máscara.** Fix real: usar `fill_ratio ≈ 0.58` (no 0.66) al encajar el bounding box del sujeto dentro del canvas del foreground — deja margen para que la semi-diagonal quede dentro del radio real de la safe zone. Verificar SIEMPRE recortando una máscara circular real sobre el compuesto foreground+background antes de dar por buena la composición (`ImageDraw.ellipse` + `Image.paste` con esa máscara en Pillow) — mirar el PNG cuadrado sin máscara no revela este problema, exactamente como un zoom nearest-neighbor no revela un apéndice mal proporcionado (regla #62) pero por una razón geométrica distinta (máscara circular vs proporción de píxeles). También: `launcher_icons/adaptive_background_432x432` es un campo real de Godot 4.7 (confirmado con WebSearch, no estaba documentado en este archivo) — si el ícono se ve con un fondo blanco/default de Godot en vez del color de marca aunque `adaptive_foreground` esté bien seteado, verificar que este campo también apunte a un PNG propio, no lo dejes vacío asumiendo que hereda el fondo del ícono plano.
68. **Asignar directamente la propiedad `.button_pressed` de un `Button` con `toggle_mode = true` SÍ vuelve a emitir la señal `toggled` (verificado en runtime con un probe — no asumido), exactamente como si lo hubiera tocado el jugador.** Si el handler de `toggled` necesita revertir programáticamente un cambio de estado (ej. una UI de selección exclusiva tipo "elegí 3 de N" donde un 4to toggle debe rechazarse y volver a `false`), asignar `button.button_pressed = false` dentro de ese mismo handler reentra la función — funciona por accidente en casos simples (la reentrada ve la condición ya resuelta y no seguiría reentrando indefinidamente), pero es frágil y confuso de razonar. Usar `button.set_pressed_no_signal(false)` en su lugar: cambia el estado visual/lógico sin volver a emitir `toggled`, evitando la reentrada por completo. Regla general: cualquier reversión programática de un control con `toggle_mode`/similar hecha DESDE DENTRO de su propio handler de cambio debe usar la variante `_no_signal` si existe, nunca la propiedad directa.
...
- **Controles:** tap ícono de torre → tap casilla libre = construir. Tap torre construida =
  panel de rango/mejora/venta. Drag vertical = paneo de cámara (camino de 10 filas — cabe
  completo en pantalla desde el ajuste post-playtesting, el drag queda como no-op inofensivo).
...
- **3 torres desbloqueables + equipo (loadout) de 3:** Chile Habanero (francotirador),
  Queso Fundido (DoT, rango corto), Pico de Gallo (multi-objetivo x3) — cada una con un
  mecanismo TÁCTICO distinto, no solo números más grandes (ver nota extensa en
  `Constants.gd`). Se compran con propinas en `WeaponShopScreen.tscn` ("ARMAS" en
  MainMenu); el equipo activo (siempre 3, default = las 3 originales) se elige en
...
| `tower_unlocked(type)` / `loadout_changed(towers)` | LoadoutManager | WeaponShopScreen / LoadoutScreen |
...
| LoadoutManager | `src/core/LoadoutManager.gd` | `user://loadout.json` — torres desbloqueadas + equipo activo de 3. Va DESPUÉS de MetaManager (unlock_tower gasta propinas). |
